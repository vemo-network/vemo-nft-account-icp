...
class TokenboundClient {
  ...
  public async prepareExecution(
    ...
    if (isDelegate) {
      executionArgs = [delegationContract ?? "", to, value, data, ""];
    }
    const executionData = encodeFunctionData({
      abi: ERC_6551_DEFAULT.IMPLEMENTATION.ABI,
      functionName: isDelegate ? "delegateExecute" : "execute",
      args: executionArgs,
    });
    return {
      to: account,
  ...
  public async transferNFT(params: NFTTransferParams): Promise<`0x${string}`> {
    ...
    if (!is1155 && amount !== 1) {
      throw new Error(
        "ERC721 transfers can only transfer one token at a time."
      );
    }
  ...
  public async transferERC20(
    ...
    if (erc20tokenDecimals < 0 || erc20tokenDecimals > 18)
      throw new Error(
        "Decimal value out of range. Should be between 0 and 18."
      );
