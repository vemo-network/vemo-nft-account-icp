import { numberToHex, TransactionReceipt } from 'viem'
...
export class SafeWalletProvider {
  ...
  async personal_sign(
    ...
    if (this.safe.safeAddress.toLowerCase() !== address.toLowerCase()) {
      throw new RpcError(
        ...
        'The address or message hash is invalid'
      )
    }
  ...
  async eth_getTransactionByHash(txHash: string): Promise<TransactionDetails> {
    ...
    return (await this.sdk.proxy('eth_getTransactionByHash', [
      txHash,
  ...
  async wallet_getCallsStatus(safeTxHash: string): Promise<{
    ...
    if (txHash) {
      receipt = await (this.sdk.proxy('eth_getTransactionReceipt', [
        txHash,
      ]) as Promise<TransactionReceipt>)
    }
